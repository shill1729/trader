...
dtfm_strategy <- function(symbol, rate = 0, kf = 1)
{
  ...
  params <- lapply(models, function(X) fit_dtfm(x, X))
  names(params) <- models
  ...
  LR_test <- numerics::likelihood_ratio(pdfs[, "stable"], pdfs[, "gmm"])
  if(length(LR_test) > 1)
  {
  ...
  kelly <- KellyCriterion::kelly_dtfm(model_fit, params[[model_fit]], r)*kf
  growth <- KellyCriterion::entropy_dtfm(model_fit, params[[model_fit]], r)
